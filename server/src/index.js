...
function bookingToClient(booking, services) {
  ...
  return {
    ...
    googleEventLink: booking.googleEventLink || "",
    calendarSynced: Boolean(booking.calendarSynced),
  };
}
...
app.get("/api/services", (req, res) => {
  ...
  const services = includeInactive
    ? data.services
    : data.services.filter((item) => item.isActive !== false);
...
app.post("/api/bookings", async (req, res) => {
  ...
  const available = ensureSlotAvailable({
    date: payload.date,
  ...
  const booking = updateData((data) => {
    const stillExists = data.services.find(
      (item) => item.id === payload.serviceId && item.isActive !== false
    );
  ...
  if (service) {
    try {
      ...
      updateData((data) => {
        ...
        item.googleEventId = calendarEvent.googleEventId;
        item.googleEventLink = calendarEvent.googleEventLink;
      ...
      booking.googleEventId = calendarEvent.googleEventId;
      booking.googleEventLink = calendarEvent.googleEventLink;
      booking.calendarSynced = true;

      console.log("Evento creado en Google Calendar:", calendarEvent.googleEventId);
    } catch (err) {
      console.error(
  ...
  res.status(201).json(bookingToClient(booking, latestData.services));
});
...
app.get("/api/admin/bookings", requireAdmin, (req, res) => {
  ...
  if (date) {
    bookings = bookings.filter((item) => item.date === date);
  }
...
app.delete("/api/admin/bookings/:id", requireAdmin, (req, res) => {
  ...
  const bookingToDelete = data.bookings.find(
    (item) => item.id === Number(req.params.id)
  );

  const service = bookingToDelete
    ? data.services.find((s) => s.id === bookingToDelete.serviceId)
    : null;
...
app.post("/api/admin/services", requireAdmin, (req, res) => {
  ...
  const service = updateData((data) => {
    if (data.services.some((item) => item.id === payload.id)) {
      return "EXISTS";
    }
  ...
  if (service === "EXISTS") {
    return res.status(409).json({ error: "SERVICE_EXISTS" });
  }
...
app.patch("/api/admin/services/:id", requireAdmin, (req, res) => {
  ...
  const service = updateData((data) => {
    ...
    if (typeof parsed.data.durationMinutes !== "undefined") {
      item.durationMinutes = parsed.data.durationMinutes;
    }

    if (typeof parsed.data.priceClp !== "undefined") {
      item.priceClp = parsed.data.priceClp;
    }

    if (typeof parsed.data.imageUrl !== "undefined") {
      item.imageUrl = parsed.data.imageUrl;
    }
