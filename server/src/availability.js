...
export function getAvailableSlots({ date, serviceId }) {
  ...
  for (let slotStart = start; slotStart + service.durationMinutes <= end; slotStart += SLOT_STEP_MINUTES) {
    ...
    const crossesBreak = breakStart !== null && breakEnd !== null && overlaps(slotStart, slotEnd, breakStart, breakEnd);
    const hasConflict = conflicts.some((item) => overlaps(slotStart, slotEnd, item.start, item.end));
    if (!crossesBreak && !hasConflict) slots.push({ startTime: minutesToTime(slotStart), endTime: minutesToTime(slotEnd) });
  }
